...
class UserServiceTest extends AnyFlatSpec with MockitoSugar{
  ...
  val userDao = UserDao(0, "test","test","test","test","test","test", Male.toString, StudentType.toString)
  val user = userDao.toUser
...
//  behavior of "login"
  ...
  it should "login correct user" in {
    when(userRepoMock.findByLoginNonBlocked(any)).thenReturn(IO.pure(Some(userDao)))
...
//  behavior of "delete"
  ...
  it should "delete valid user" in {
    when(sessionRepoMock.getIdBySession("valid session")).thenReturn(IO.pure(Some(user.id)))
  ...
  it should "throw Exception with invalid session on delete" in {
    when(sessionRepoMock.getIdBySession("invalid session")).thenReturn(IO.pure(Option.empty[Long]))
    when(userRepoMock.deleteById(any)).thenReturn(IO.pure(1))
  ...
  it should "throw Exception with invalid user id" in {
    when(sessionRepoMock.getIdBySession("valid session")).thenReturn(IO.pure(Option.empty[Long]))
    when(userRepoMock.deleteById(any)).thenReturn(IO.pure(1))
...
//  behavior of "get user by id"
  ...
  it should "throw Exception with invalid session on get" in {
    when(sessionRepoMock.getIdBySession("invalid session1")).thenReturn(IO.pure(Option.empty[Long]))
    when(userRepoMock.getUserById(any)).thenReturn(IO.pure(Some(userDao)))
